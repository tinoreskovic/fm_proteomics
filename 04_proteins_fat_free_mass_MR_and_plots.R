...
if(restarted){
  fat_free_mass_regenie <- fread(file.path(controlled_access_dir, "fat_free_mass_gwas_regenie.txt"))
  ...
  remove(ukb_mfi)
  fat_free_mass_regenie <- subset(fat_free_mass_regenie, fat_free_mass_regenie$ID %in% info_07)
  remove(info_07)
  gc()
...
mr_results_file <- paste0(output_folder, prefix, "fat_free_mass_mr_results_", clump_r2_name, "_", pc_thresh_name, ".csv")
mr_results_by_clump_file <- paste0(output_folder, prefix, "fat_free_mass_mr_results_", clump_r2_name, ".csv")
pca_gmm_results_file <- paste0(output_folder, prefix, "fat_free_mass_pca_gmm_results_", pc_thresh_name, ".csv")
fm_layout_file <- paste0(output_folder, prefix, "fat_mass_forest_plot_layout.rds")
ffm_layout_file <- paste0(output_folder, prefix, "fat_free_mass_forest_plot_layout.rds")
lax_fm_mr_results_file <- paste0(output_folder, prefix, "fat_mass_mr_results_4_", pc_thresh_name, ".csv")
lax_ffm_mr_results_file <- paste0(output_folder, prefix, "fat_free_mass_mr_results_4_", pc_thresh_name, ".csv")
...
if(run==T){
  ...
  for(id in ids){try({
    ...
    if(chr=="X"){
      outcome <- subset(fat_free_mass_regenie, fat_free_mass_regenie$CHROM==23)
    }else{
      outcome <- subset(fat_free_mass_regenie, fat_free_mass_regenie$CHROM==as.numeric(chr))
    }
    ...
    outcome$id.outcome <- "Whole body fat-free mass"
    ...
    rsqrs_df <- rsqrs_df[!duplicated(rsqrs_df$exposure), ]
	    ...
	    write.csv(rsqrs_df, paste0(output_folder, prefix, "fat_free_mass_steiger_directionality_", clump_or_pc,  ".csv"), row.names = FALSE)
    ...
    if(length(dat$SNP) == 1){
      ...
      mro$id.exposure <- id
      mro$exposure <- id
      mro$id.outcome <- "Whole body fat-free mass"
      mro$outcome <- "outcome"
      ...
      mro_ffm <- rbind(mro_ffm, mro)
      mro_ffm <- subset(mro_ffm, !is.na(mro_ffm$b))
      
	      write.csv(mro_ffm, mr_results_by_clump_file)
  ...
  write.csv(no_cis_pqtls, paste0(output_folder, prefix, "fat_free_mass_no_genome_wide_cis_pqtls_", clump_r2_name, ".csv"))
  write.csv(two_or_fewer_pqtls, paste0(output_folder, prefix, "fat_free_mass_two_or_fewer_cis_pqtls_", clump_r2_name, ".csv"))
...
if(run==T){
  ...
  for(id in unique(no_cis_pqtls)){
      ...
      if (length(dat$SNP) >= 1) {
        tryCatch({
          ...
          dat <- MendelianRandomization::mr_input(
            ...
            correlation = matrix(),
            exposure = id,
            outcome = "Whole body fat-free mass",
          ...
          mro_ffm_pc <- rbind(mro_ffm_pc, mro_dat)
          write.csv(mro_ffm_pc, pca_gmm_results_file)
    ...
    message("Finished processing for id: ", id, " at ", iteration_end_time, " (Duration: ", duration, " seconds)")
	  ...
	  mro_ffm$code <- tolower(mro_ffm$exposure)
  mro_ffm$PCs <- NA
  mro_ffm$fstat <- NA
  mro_ffm$hetr_stat <- NA
  mro_ffm$ci_lwr <- mro_ffm$b - (1.96*mro_ffm$se)
  mro_ffm$ci_upr <- mro_ffm$b + (1.96*mro_ffm$se)
  
  mro_ffm_pc$code <- tolower(mro_ffm_pc$exposure)
  mro_ffm_pc$id.exposure <- mro_ffm_pc$exposure
  mro_ffm_pc$id.outcome <- mro_ffm_pc$outcome
  mro_ffm_pc$method <- "PCA-GMM"
  mro_ffm_pc$se <- (mro_ffm_pc$ci_upr - mro_ffm_pc$b)/1.96
  
  mro_ffm_pc$direction <- NA
  mro_ffm_pc$direction_p <- NA
  
  
  mro_ffm <- rbind(mro_ffm, mro_ffm_pc)
  
  mro_ffm <- subset(mro_ffm, !is.na(mro_ffm$b))
  
  
  mro_ffm$method_group <- ifelse(mro_ffm$method == "Wald ratio" |
                                  mro_ffm$method == "PCA-GMM" |
                                  mro_ffm$method == "Inverse variance weighted", "main", mro_ffm$method)
  
  if(clump_or_pc=="strict"){
    
    mro_ffm <- subset(mro_ffm, mro_ffm$method!="PCA-GMM")
  ...
  mro_ffm$code <- tolower(mro_ffm$exposure)
  mro_ffm <- merge(mro_ffm, coding143_edited, by.x = "code", by.y = "code", all.x = TRUE)
  
  write.csv(mro_ffm, mr_results_file)
...
#reading in MR
if(run==F){
  mro_ffm <- read.csv(mr_results_file)
}

mro_ffm$method <- ifelse(mro_ffm$method == "PC-GMM", "PCA-GMM", mro_ffm$method)
...
sig_proteins <- subset(mro_ffm, (mro_ffm$fdr < 0.05 & mro_ffm$method=="Inverse variance weighted") |
                         (mro_ffm$fdr < 0.05 & mro_ffm$method=="Wald ratio") |
                         (mro_ffm$fdr < 0.05 & mro_ffm$method=="PCA-GMM")
)

sig_proteins <- unique(sig_proteins$exposure)

pleiotropic_total <- unique(subset(mro_ffm, mro_ffm$fdr < 0.05 & mro_ffm$method == "MR Egger intercept")$exposure)

egger_sig <- unique(subset(mro_ffm, mro_ffm$fdr < 0.05 & mro_ffm$method=="MR Egger")$exposure)

pleiotropic <- setdiff(pleiotropic_total, egger_sig)

mro_ffm_concordance <- subset(mro_ffm, (method!="MR Egger intercept") &
                               (method!="MR Egger" |
                                  mro_ffm$exposure %in% pleiotropic_total))

mro_ffm_concordance$positive <- ifelse(mro_ffm_concordance$b > 0, 1, 0)
mro_ffm_concordance <- mro_ffm_concordance %>% group_by(exposure) %>% 
  dplyr::mutate(positive_sum = sum(positive))
mro_ffm_concordance <- mro_ffm_concordance %>% group_by(exposure) %>% 
  dplyr::mutate(num_methods = n())
mro_ffm_concordance$positive_ratio <- mro_ffm_concordance$positive_sum/mro_ffm_concordance$num_methods
concordant <- mro_ffm_concordance %>% filter(positive_ratio==1 | positive_ratio==0)
concordant <- unique(concordant$exposure)

sig <- mro_ffm %>% filter(!(exposure %in% pleiotropic) & (exposure %in% sig_proteins) & (exposure %in% concordant))

sig_main <- mro_ffm %>% filter(!(exposure %in% pleiotropic) & (exposure %in% sig_proteins) & (exposure %in% concordant))
...
if(T){
  try({
    ...
    volcano_data <- subset(mro_ffm, mro_ffm$method=="Inverse variance weighted" | mro_ffm$method=="Wald ratio" | mro_ffm$method=="PCA-GMM")
...
if(proteins=="selected"){
  ...
  mro_ffm$"95% CI" <- paste0(round(mro_ffm$ci_lwr, 2), " - ", round(mro_ffm$ci_upr, 2))
  mro_ffm$"SNPs/PCs" <- ifelse(mro_ffm$method == "PCA-GMM", mro_ffm$PCs, mro_ffm$nsnp)
  
  if("X" %in% colnames(mro_ffm)){
    mro_ffm <- subset(mro_ffm, select=-c(X))
  }
  ...
  primary_mr_summary <- subset(mro_ffm, mro_ffm$method=="Inverse variance weighted" | mro_ffm$method=="Wald ratio" | mro_ffm$method=="PCA-GMM") 
  ...
  egger_intercept_summary <- subset(mro_ffm, mro_ffm$method=="MR Egger intercept") 
  ...
  write.csv(primary_mr_summary, paste0(output_folder, prefix, "fat_free_mass_primary_mr_summary_", clump_r2_name, "_", pc_thresh_name, ".csv"))
  
}

#plotting/summary forest plot
if(T){
  lax_fm_results <- data.frame()
  if(file.exists(lax_fm_mr_results_file)){
    lax_fm_results <- read.csv(lax_fm_mr_results_file)
    lax_fm_results <- subset(
      lax_fm_results,
  ...
  if(proteins=="selected" & clump_or_pc=="lax"){
    plot_dta <- subset(mro_ffm, mro_ffm$method %in% c("Inverse variance weighted", "Wald ratio", "PCA-GMM"))}
  
  if(proteins=="selected" & clump_or_pc=="strict"){
    lax_sig_ids <- if(nrow(lax_fm_results) > 0) unique(lax_fm_results$exposure) else unique(mro_ffm$exposure)
    mro_ffm$fdr <- round(mro_ffm$fdr, 2)
    plot_dta <- subset(mro_ffm, (mro_ffm$method=="Inverse variance weighted" |
                                   mro_ffm$method=="Wald ratio")
                       & (mro_ffm$exposure %in% lax_sig_ids)
    )}
  ...
  if(!file.exists(fm_layout_file)){
    stop("Run 03_proteins_fat_mass_MR_and_plots.R first so the FM forest-plot order is available.")
  }
  fm_layout <- readRDS(fm_layout_file)
  protein_order_FM <- fm_layout$order
  
  if(proteins=="selected"){
    plot_dta <- subset(plot_dta, id.exposure %in% protein_order_FM)
  }
  plot_dta <- plot_dta %>%  
    mutate(exposure = factor(exposure, levels = protein_order_FM)) %>%  
    arrange(match(exposure, protein_order_FM))
  
  # Prepare labels (modify as needed for your data)
  plot_dta$key <- plot_dta$id.exposure
  
  
  ffm_axis_limits <- make_forest_axis_limits(plot_dta)
  if(exists("fm_layout") && !is.null(fm_layout) && !is.null(fm_layout$xlim)){
    xlim_vals <- c(min(fm_layout$xlim[1], ffm_axis_limits[1]),
                   max(fm_layout$xlim[2], ffm_axis_limits[2]))
  }else{
    xlim_vals <- ffm_axis_limits
  }
  xticks_vals <- seq(from = xlim_vals[1], to = xlim_vals[2], by = 0.5)
  saveRDS(list(order = plot_dta$id.exposure, xlim = xlim_vals, xticks = xticks_vals), file = ffm_layout_file)
  ...
  ggsave(paste0(output_folder, "plots/", prefix, 'panel_protein_to_fat_free_mass_mr_forest_plot_', clump_r2_name, "_", pc_thresh_name, "_ffm", '.pdf'), 
         width = ifelse(nrow(plot_dta)>3,
                        8, 5), height = ifelse(nrow(plot_dta)>6,
                                               nrow(plot_dta)*0.6, nrow(plot_dta)),
         unit = 'in', dpi = 500)
  
  ggsave(paste0(output_folder, "plots/", prefix, 'wide_panel_protein_to_fat_free_mass_mr_forest_plot_', clump_r2_name, "_", pc_thresh_name, "_ffm", '.pdf'), 
         width = ifelse(nrow(plot_dta)>3,
